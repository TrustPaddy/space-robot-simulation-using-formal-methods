...
In micro-gravity, the robot’s base (typically a spacecraft) is not fixed and thruster are turned off. Manipulator motions can give rise to a change of the position and orientation of the base, which in turn modifies the final pose of the end-effector. This reaction is due to the conservation of the linear and angular momentum. The free-floating dynamics of space robots has many benefits. An expensive fuel (e.g., hydrazine ($N_2H_4$)) consumption  and  excitation of the robot dynamics for some activation frequencies of the pulse width modulation for thruster control can be avoided \cite{papadopoulos2007minimum}.

However, a moving base complicates the achievement of robotized manipulations because the end-effector of the arm may fail to reach the desired target point. Further constrains, such as communication latency and limited onboard computational resources, complicate the real-time control and stabilization of the base. Traditional control
approaches that assume an accurate model of the robot dynamics (e.g. reactionless planning or precise attitude control) 
can struggle to adapt to uncertainties or unmodeled dynamics. Data-driven approaches, on the other hand, hardly apply because the required amount and quality of data are rather unavailable in the space sector. In many cases, robotized tracking capabilities under a free-floating behavior need to be demonstrated long before the physical system is assembled and deployed.  These challenges motivate the
exploration of learning-based control methods that can autonomously adapt to complex and uncertain dynamics in virtual environments even in the case of data scarcity.
\\ \\
...
instability in highly coupled or safety-critical environments.
...
space robot control, clipping contributes significantly to stable and reliable convergence.
...
\bibitem{b10}
ResearchGate, ``Space robot with a 4-DOF robotic arm,'' Scientific diagram.
[Online]. Available: \underline{https://www.researchgate.net/figure/Space-robot-with-a-4-DOF-robotic-arm\_fig5\_321286132}
