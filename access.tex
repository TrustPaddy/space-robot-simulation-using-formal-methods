...
base’s orientation using a using quaternions. The quaternion \( q_{base} \in \mathbb{R}^4 \) describes the orientation of the base, capturing its attitude 
...
% \end{itemize}
\begin{itemize}
  \item K1: Average Episodic Return, the mean cumulative reward per episode (lower is better, this reflects overall success in the task).\\
  ...
  \item K4: Mean Base Orientation Error, e.g., average quaternion error magnitude (in radians) indicating how far the base drifted in attitude (lower is better).\\
...
\hline
\makecell[l]{\textbf{K2:} Mean End Effector (EE)\\position error} &
