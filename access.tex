...
\begin{abstract}
Free-floating space robots can exhibit a strong dynamic coupling between the manipulator motion and the  base attitude disturbance. 
This complicates the usage of these robots to track a desired Cartesian path of the end-effector. 
Adopting data-driven approaches to this end is challenging because the quantity and quality of data are rather hard to acquire in the space sector.
This paper therefore explores the performance of  trial-and-error learning techniques employed for Cartesian path tracking in a physics-based
...
missing in the on-orbit servicing realm, we benchmark six Reinforcement Learning agents, i.e., TRPO, TD3, SAC, PPO,
Policy Gradient, and DDPG. We use each of them to track the same path of the end-effector and evaluate their performance using a unified set
of key performance indexes related to e.g. the training duration and stability, tracking accuracy, base attitude disturbance, motion smoothness, energy consumption, and collision risks. 
We observe that PPO achieves the best overall
trade-off between tracking accuracy, minimization of the base disturbance, as well as training stability and efficiency.
Building on this result, we quantify the effect of a systematic tuning of neural network architecture and key hyperparameters of PPO for tracking 
both a circular and ramp-shaped Cartesian path of the end-effector.
Compared to a default PPO configuration, the optimized PPO improves the mean episodic return magnitude by 80.7\%,
reduces the mean squared end-effector tracking error by 67.7\% and the maximum tracking error by 44.1\%, and stabilizes
the base orientation (67.0\% reduction in orientation error). Moreover, the optimized PPO eliminates collisions and joint-limit
violations in evaluation while slightly reducing energy consumption (15.98\%).
\end{abstract}

\begin{keywords}
space robotics, free-floating manipulation, reinforcement learning, PPO, safe simulation, trajectory tracking
\end{keywords}
...
Free-floating robot manipulators operating in the space environment to inspect, capture, or de-orbit  satellites 
...
An expensive consumption of fuel, such as hydrazine ($N_2H_4$),  and  excitation of the robot dynamics for some activation 
frequencies of the pulse width modulation related to thruster control can be avoided \cite{b11}.
...
demonstrated long before the physical robot system is assembled and deployed \cite{muhlbauer2025software,rybus2024robotic,maurenbrecher2024robograv}.  
These challenges motivate the development and simulation of AI-empowered digital twins under desired operational conditions to
...
\begin{table*}[h]
	...
	\begin{tabular}{|c|c|c|c|}
		...
		(\cite{wei2025trajectory},~2025) & DDPG & Trajectory planning  &  No$\slash$Yes \\ \hline
	\end{tabular}
	\caption{Recent advances in Deep RL-based motion management for free-floating space robots.}
	\label{tab:sample}
\end{table*} % wei2025trajectory
...
(PPO) \cite{b15}, vanilla Policy Gradient (PG)\cite{b21}, and Deep Deterministic Policy Gradient (DDPG) \cite{b7}, on the same free-floating robot control task. 
This side-by-side comparison under unified
conditions reveals which algorithm is best suited for controlling a free-floating space robot and
why.
...
ensures the RL agent cannot violate fundamental safety properties (like avoiding collisions or exceeding actuator limits) during training or execution.
...
a specially designed reward function that incentivizes accurate end-effector trajectory tracking while penalizing base movement, 
...
have begun exploring RL for path planning \cite{wu2020reinforcement,al2024path}, 
...
\begin{figure*}[t]
    ...
    \begin{subfigure}{0.99\columnwidth}
        \centering
        \includegraphics[width=0.99\columnwidth]{Figures/ref_kreisbahn_1.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.99\columnwidth}
        \centering
        \includegraphics[width=0.99\columnwidth]{Figures/ref_kreisbahn_2.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}{1.\columnwidth}
        \centering
        \includegraphics[width=1.2\columnwidth]{Figures/ref_kreisbahn_3.png}
    \end{subfigure}
...
uncertainties, digital twins bridge the physical and digital world with accelerated simulation-driven analysis at low costs. Verification 
...
DoF joint \cite{b3}, allowing it to move freely in translation and rotation. No external forces or torques act on
the system. Thus, the base only moves in response to forces generated by the manipulator’s joints. This
configuration ensures that momentum is conserved, motions of the arm can lead to a reaction of the base \cite{b3,b6,b28,b34}.

\begin{figure*}[!t]
	\centering
	\includegraphics[width=1\linewidth]{Figures/spacerobot_slx_robot.png}
...
The Simulink model is organized into modular subsystems, as illustrated in~\autoref{fig:spacerobot_slx}. The \emph{Robot Plant} subsystem computes 
...
and outputting the resulting states by numerically integrating the equations of motion~\cite{b6}. The \emph{Controller} subsystem executes the learned RL policy, 
...
the RL agent (detailed in \cref{RL}). Finally, \emph{safety monitors} enforce constraints: a Collision Monitor checks distances between robot components and triggers 
...
\begin{figure*}[!t]
	\centering
	\includegraphics[width=\linewidth]{Figures/spacerobot_slx_reference.png}
...
\noindent
Reward Function Design: A crucial aspect of the reinforcement learning formulation is the reward signal, which guides the agent toward accurate trajectory tracking 
...
the episode is immediately terminated and a terminal reward of $-1$ is assigned. This mechanism prevents the agent from exploiting invalid states and ensures numerical robustness during training.
...
\begin{figure}[b]
  \centering
  \includegraphics[width=1\linewidth]{Figures/impulserhaltung_scope.png}
...
\begin{figure*}[!t]
    \centering    \includegraphics[width=1\linewidth]{Figures/spacerobot_slx_colissionmonitor.png}
...
\begin{figure*}[t] 
    ...
    \begin{subfigure}[b]{0.99\columnwidth}
        \centering
        \includegraphics[width=0.99\columnwidth]{Figures/warning_collision.png}
    ...
    \begin{subfigure}[b]{0.99\columnwidth}
        \centering
        \includegraphics[width=0.99\columnwidth]{Figures/tau_collision.png}
...
Training Setup: All experiments were conducted on a workstation equipped with an AMD Ryzen 7 7700 processor 
...
stability, etc.) by running 30 test episodes for each.~\autoref{tab:kpi_comp} below summarizes a subset of these metrics,
...
\hline
\makecell[l]{\textbf{K2:} Mean Squared End Effector (EE)\\position error} &
0.0680 & 0.1914 & 0.1908 & \textbf{0.0257} & 0.1638 & 0.0937 \\
\hline
\makecell[l]{\textbf{K3:} Max EE error\\} &
...
\begin{figure*}[t]
	\centering
	\begin{subfigure}{1.\columnwidth}
		\includegraphics[width=0.99\columnwidth]{Figures/rainingsverlauf_ppo_default.png}
		\caption{PPO (Default)}
		\label{fig:training_ppo_default}
	\end{subfigure}
	\hfill
	\begin{subfigure}{1.\columnwidth}
		\includegraphics[width=0.99\columnwidth]{Figures/trainingsverlauf_ppo_optimized.png}
...
\begin{figure*}[t]
    ...
    \begin{subfigure}{0.9\columnwidth}
        \includegraphics[width=0.9\columnwidth]{Figures/soll_vs_ist_kreisbahn.png}
        \caption{PPO (Default)}
        \label{fig:traj_default}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.9\columnwidth}
        \includegraphics[width=0.9\columnwidth]{Figures/soll_vs_ist_kreisbahn_opt.png}
        \caption{PPO (Optimized)}
        \label{fig:traj_opt}
    \end{subfigure}
...
\begin{figure*}[t]
    \centering
    \begin{subfigure}{0.9\columnwidth}
        \includegraphics[width=0.9\columnwidth]{Figures/basis_orientaion_ppo.png}
        \caption{PPO (Default)}
        \label{fig:basis_default}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.9\columnwidth}
        \includegraphics[width=0.9\columnwidth]{Figures/basis_orientation_ppo_optimized.png}
        \caption{PPO (Optimized)}
        \label{fig:basis_opt}
    \end{subfigure}
...
and energy efficiency were all achieved simultaneously, indicating a fundamentally more efficient control strategy.
...
\begin{figure*}[t]
    \centering
    \begin{subfigure}{0.9\columnwidth}
        \includegraphics[width=0.9\columnwidth]{Figures/rampe_1.png}
        \caption{PPO (Default)}
        \label{fig:r_traj_default}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.9\columnwidth}
        \includegraphics[width=0.9\columnwidth]{Figures/rampe_opt_1.png}
        \caption{PPO (Optimized)}
...
\begin{figure*}[t]
    \centering
    \begin{subfigure}{0.9\columnwidth}
        \includegraphics[width=0.9\columnwidth]{Figures/basis_ori_1.png}
        \caption{PPO (Default)}
        \label{fig:r_basis_default}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.9\columnwidth}
        \includegraphics[width=0.9\columnwidth]{Figures/basis_ori_opt_1.png}
        \caption{PPO (Optimized)}
...
\textbf{Metric (KPI)} & \textbf{PPO (Default)} & \textbf{PPO (Optimized)} \\
\hline
Mean Episodic Reward (K1)            & -0.4565 & \textbf{-0.2977} \\
...
% \textbf{Evaluation on Piecewise-Linear End-Effector Trajectories:}
% While the previous evaluations focused on tracking a smooth circular end-effector (EE) reference, we 
% additionally tested how well the controller generalizes to \emph{linear} trajectories.
% This is relevant for practical on-orbit manipulation tasks, where approach and retreat motions are frequently 
% planned as straight-line segments rather than continuous curves.
...
sensor noise, actuator latency, and external disturbances (gravity gradients, magnetic torques) could degrade real-world performance~\cite{b8,b22}. 
...
or barrier functions~\cite{b22,b24,b23,b34}; and (iv)~hardware-in-the-loop validation on robotic testbeds with simulated microgravity (e.g., air-bearing tables).
...
M. M{\"u}hlbauer, M. Chalon, M. Ulmer, and A. Albu-Sch{\"a}ffer, ``Software for the SpaceDREAM robotic arm,'' in \emph{Proc. IEEE Aerosp. Conf.}, 2025, pp. 1--10.

\bibitem{rybus2024robotic}
T. Rybus, ``Robotic manipulators for in-orbit servicing and active debris removal: Review and comparison,'' \emph{Prog. Aerosp. Sci.}, vol.~151, p.~101055, 2024.
