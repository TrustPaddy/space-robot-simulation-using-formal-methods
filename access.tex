...
we performed an ablation study in which the optimized PPO agent was retrained on the circular trajectory with each weight in turn
set to zero while keeping all remaining weights at their baseline values ($W_p=150$, $W_v=25$, $W_{\mathrm{ori}}=200$, $W_{wb}=8$,
$W_{vb}=2$, $W_u=0.02$, $W_d=0.06$). All other aspects of the training and evaluation pipeline (hyperparameters, random seeds,
episode horizon, and the $N_{eval}=30$ evaluation protocol) were held identical to the baseline configuration.
The resulting KPIs are summarized in~\autoref{tab:reward_ablation}.
\end{rtwohl}
...
\noindent\textbf{Summary.} The ablation confirms that $W_{\mathrm{ori}}$ and $W_p$ are the two dominant components of the composite
reward, while the remaining five weights contribute refinements in motion quality, base velocity damping, and actuation smoothness.
Removing any single secondary weight still yields a functioning, if degraded, policy, whereas removing $W_{\mathrm{ori}}$ breaks
the free-floating controller entirely. This supports the reward design choices reported in~\autoref{RL} and shows that the
benchmark conclusions do not rely on fine balancing of low-weight terms.
\end{rtwohl}
