...
\title{Benchmarking Reinforcement Learning for Trajectory Tracking of Free-Floating Space Robot Manipulators:
Quantifying the Impact of Hyperparameter Optimization for Proximal Policy Optimization Using a Simulated Digital Twin}
...
This complicates their motion tracking capabilities. Data-driven approaches are hardly provided with the required  quantity 
and quality of data to develop control techniques that can accommodate uncertainties inherent to the robot dynamics and the space environment \cite{b1,b2,b22}.
...
However, a moving base complicates the achievement of robotized manipulations because the end-effector of the arm may 
fail to reach the desired target point. Further constraints, such as communication latency and limited onboard computational resources, 
complicate the real-time control and stabilization of the base \cite{b1,b32}. Traditional control
approaches that assume an accurate model of the robot dynamics (e.g. reactionless planning or precise attitude control) 
can struggle to adapt to uncertainties or unmodeled dynamics. Data-driven approaches, on the other hand, 
hardly apply because the required amount and quality of data are rather unavailable in the space sector. In many cases, 
robotized tracking capabilities under a free-floating behavior need to be demonstrated long before the physical system is assembled and deployed.  
These challenges motivate the exploration of learning-based control methods that can autonomously adapt to 
complex and uncertain dynamics in virtual environments even in the case of data scarcity.
\\ \\
Reinforcement Learning (RL) has emerged as a promising approach for robotics, enabling agents to
learn control policies through trial-and-error interactions \cite{b2,b7}. Recent advances in deep RL have shown
...
% ====== NEU: Ergänzende Primär-/offizielle Quellen (empfohlen) ======
