...
Indeed, the end-effector of the arm may fail to reach the desired target point if standard motion planning approaches dedicated to robots with a fixed base are simply applied. Further constrains, such as communication latency 
...
manipulator. It differs from previous contribution in three ways. First, it focuses on a systematical comparison 
and explores the respective performance of different RL algorithms beyond those mentioned e.g. in \cref{tab:sample}, thereby quantitatively and qualitatively contributing to filling in a current gap in space robotics. Second,  constrains, such as collision avoidance, are monitored
...
penalties for unsafe actions. This reward function is initalised inside a Matlabfunction of the Reward Subsystem (see~\autoref{fig:reward_fcn}). The reward is essentially the negative of a weighted cost function that
...
\centering
   \includegraphics[width=1\linewidth]{Figures/impuls_monitor.png}
    \caption{Overview of the Impuls Monitor Subsystem in the Simulink Model \texttt{SpaceRobot.slx} \\
...
\begin{figure*}[t]
    ...
    \end{subfigure}
    \caption{Course of basic orientation (quaternion components) during a test episode with ramp trajectory.}
