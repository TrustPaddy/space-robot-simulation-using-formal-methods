...
\begin{abstract}
Free-floating space robots exhibit a strong dynamic coupling between their manipulator motion and base attitude disturbance, 
complicating the Cartesian path tracking of the end-effector. 
Supervised data-driven motion control approaches are challenging due to limited  availability of labeled data from space operations. This paper therefore explores trial-and-error learning techniques for Cartesian path tracking of free-floating space robots in a physics-based Simulink/Simscape environment with integrated safety monitoring. Since a quantitative and qualitative comparison of these methods is currently 
...
\begin{figure}[!t]
	...
	\end{subfigure}
	\caption{Virtual testbeds that leverage AI-driven digital twins are useful for the safe, flexible, and accelerate development of learning-based control methods that support  in-space operations under realistic yet controllable conditions.}
	\label{fig:oos0}
\end{figure}
...
frequencies of the pulse width modulation related to thruster control can be avoided \cite{b11}. This measure has the potential to extend the lifetime of space robots and enhance the mission success.
...
\textcolor{magenta}{These challenges motivate the development and simulation of AI-empowered digital twins (DTs) \cite{kaigom2025deep}, as illustrated in \cref{fig:oos0}. Unlike standard DTs driven by sensed real-time data from their physical counterparts, such DTs are designed to infer and contextualize system states, reason, and evolve under representative distributions of operational conditions. These include uncertain dynamics, disturbances, faults, and contact-rich interactions associated with satellite capture in the space environment. By enabling accelerated experimentation through faster-than-real-time simulation and systematic ablation studies, DTs can improve decision making and help reduce the simulation-to-reality gap. They provide a framework to explore and design robust learning-based control methods that support safe exploration, robust policy optimization, and domain randomization for sim-to-real generalization, thereby enabling autonomous adaptation to complex and unknown dynamics even under severe data scarcity. This is particularly valuable for space robotics, where limited flight data, safety-critical constraints, and costly on-orbit experimentation make AI-driven DTs an essential tool for training, validation, and policy transfer.}



\begin{table*}[!t]
	\centering
	\caption{\hltwo{[R2.3]} Recent advances in Deep RL-based motion management for free-floating space robots. N/R\,=\,not reported. ,\textcolor{red}{Eine Bescheibung der Abkürzungen fehlt.}}
	\label{tab:sample}
	\scriptsize
...
This paper investigates the use of continuous-action deep RL for the tracking of a Cartesian motion of the end-effector of a free-floating space
manipulator. It  differs from previous contributions in three ways. First, it focuses on a systematic comparison 
and explores the respective performance of six different RL algorithms (see \cref{comparison}) by conducting ablation studies (see \cref{sec:reward_ablation}) beyond the scope of related work mentioned in \Cref{tab:sample}. It thereby  quantitatively and qualitatively contributes to filling in a current gap in space robotics. Second, several constraints, such as collision avoidance, are monitored
through formal methods applied from a software-enabled and practical perspective (see, \cref{formal}).  Finally, this work investigates the minimization of the base attitude during arm motions in conjunction with constraint monitoring previously mentioned. Recall that the induced base disturbance is omitted in most recent contributions, 
...
we incorporate formal methods into the learning loop \cite{b5, b6, b22, b23}. Collision monitoring, momentum
...
ensures the RL agent cannot violate fundamental and critical safety properties (like avoiding collisions or exceeding actuator limits) during training and testing.
 \item A versatile simulation environment for multi-criteria optimization in in-space servicing: We develop a flexible MATLAB/Simulink simulation framework for a free-floating space robot, based on a URDF-derived multibody model and the Simscape physics engine. The robot, a  cube-shaped base with a 4-Degree of Freedom (DOF) robotic arm, is modeled with realistic kinematics, dynamics, and sensor/actuator 
...
excessive base disturbance or safety violations \cite{b25,b26,b27,b28,b7}.
 \item The optimization of the PPO agent : Based on  comparative results, we identify PPO as the top-
...
\end{itemize}
%\newpage
Our investigation shows that the integration of multibody simulation, deep reinforcement learning, and formal verification can yield robust and safe control policies for free-floating space robots. In the following, we first describe the system modeling and reinforcement learning framework, then detail the formal safety components, present a comparative analysis of the reinforcement learning agents, and finally discuss the performance of the optimized PPO agent before concluding with broader insights and future research directions.
...
The key advantage of RL is its ability to synthesize controllers without explicit dynamic models and requiring flight data, making it attractive when system
...
evaluate only a single algorithm, focus on an objective, omit base attitude minimization as a control objective, and lack systematic benchmarking across comparable conditions.
\end{rtwohl}
...
three gaps simultaneously. It benchmarks six state-of-the-art RL algorithms under unified conditions, incorporate base
...
\begin{figure*}[!t]
 ...
 %   \hfill
    ...
    \end{subfigure}
    \caption{Tracking the desired end-effector circular path using inverse kinematics. \textcolor{blue}{Das Bild oben links zeigt einen Manipulator in einer singulären Lage. Ich würde nur  die Bilder oben rechts und unten nehmen und nebeneinander platzieren.}}
...
Additionally,  consistency checks of Simulink’s units are enabled to catch any modeling errors (e.g.
...
\caption{Pairwise Wilcoxon signed-rank tests: each agent vs.\ PPO (baseline), Holm-corrected.
  ...
  For K2--K4, K6--K8, negative $\Delta\tilde{x}$ means lower (better) error/violation/jerk.
  K5 is a binary collision flag (1\,=\,safe); K9 lower means more energy-efficient.}
...
\midrule
TRPO & $+7.55^{*}$    & $-0.277^{***}$ & $-0.526^{***}$ & $+0.003^{\mathrm{n.s.}}$ & $-1^{***}$    & $-0.137^{***}$ & $+0.362^{***}$ & $-0.041^{***}$ & $-9.38^{***}$  \\
TD3  & $+8.90^{***}$  & $+0.047^{\mathrm{n.s.}}$ & $-0.276^{***}$ & $-0.097^{***}$ & $-1^{***}$    & $-0.156^{***}$ & $+0.731^{***}$ & $-0.095^{***}$ & $-12.91^{***}$ \\
SAC  & $-33.79^{***}$ & $+0.908^{***}$ & $+0.981^{***}$ & $+0.093^{**}$  & $0^{\mathrm{n.s.}}$   & $-0.059^{***}$ & $+0.404^{***}$ & $+0.032^{***}$ & $+2.44^{*}$    \\
PG   & $-24.46^{*}$   & $-0.051^{\mathrm{n.s.}}$ & $-0.132^{\mathrm{n.s.}}$ & $+0.158^{**}$  & $-1^{***}$    & $-0.156^{***}$ & $+0.199^{***}$ & $0.000^{\mathrm{n.s.}}$  & $-4.05^{***}$  \\
...
To substantiate the qualitative ranking above with rigorous statistical evidence, we applied
non-parametric hypothesis tests to the 30 evaluation episodes per agent.
...
The collision-rate KPI (K5) is identically zero for TRPO, TD3, and PG, resulting in a
statistically significant disadvantage compared to PPO ($p<0.001$, $\Delta\tilde{x}=-1$),
which achieves K5\,=\,1 consistently due to its exploration of the workspace.
No significant difference is found for DDPG and SAC (both also achieve K5\,=\,1), confirming
that collision avoidance is maintained equally well across on-policy and off-policy stochastic
methods.
...
\begin{roverlaphl}
\hloverlap{[R2.5/R3.4]} The ablation study in~\autoref{sec:reward_ablation} establishes which reward components are
structurally load-bearing. A complementary question, raised independently by both reviewers, is whether the
...
ranking of the algorithms remain qualitatively stable? To answer this, we performed a one-at-a-time sensitivity
analysis on the four dominant cost weights identified by the ablation, namely $W_p$, $W_v$, $W_{\mathrm{ori}}$, and
...
\begin{table}[!t]
    ...
    \begin{tabular}{lcccccc}
        ...
        $W_{\mathrm{ori}} \times 2$         & $\mathbf{-6.89}$ & $\mathbf{0.2221}$ & $\mathbf{0.907}$  & $0.041$          & $0.622$          & $3.059$ \\
...
Halving $W_{\mathrm{ori}}$, in contrast, relaxes the base-stabilization pressure without causing catastrophic
behavior — the agent still tracks the reference ($K2=0.0438$) but with twice the baseline base-orientation error
($K4=0.050$ vs.\ baseline $0.022$). This is fully consistent with the ablation conclusion that $W_{\mathrm{ori}}$
is the structurally dominant cost weight.
...
consistent with the ablation finding that $W_p$ is the second most load-bearing weight.
...
\noindent\textbf{Safety guarantees remain intact.} Across all eight perturbed configurations the collision rate
remains at $K5=0$ and the joint-limit violation rate stays below $0.53\,\%$ ($K6 \le 0.0053$). The formal
safety-monitoring layer introduced in~\autoref{formal} therefore decouples the constraint-enforcement guarantees
from the exact numerical tuning of the reward weights, which is an important robustness property for an
on-orbit-servicing controller: reward miscalibration degrades task performance but does not compromise hard
safety limits.
...
in the worst-case configuration ($W_{\mathrm{ori}}\times 2$) the resulting EE tracking error ($K2=0.222$) is not
worse than the \emph{optimally-tuned} TD3 and SAC baselines reported in~\autoref{tab:kpi_comp} ($K2 \approx 0.19$
...
budget by a factor of six and is beyond the scope of this benchmark; however, the result reported here is
sufficient to establish that the ranking conclusions are not an artifact of a brittle, precisely-tuned operating
point.
...
\noindent\textbf{Summary.} The sensitivity analysis confirms the reward-design choices reported in~\autoref{RL}:
$W_{\mathrm{ori}}$ is the most sensitive weight, $W_p$ is secondary, and $W_v$ and $W_{wb}$ are comparatively
insensitive. Task solvability and all hard safety limits are preserved across the full $\times 0.5$ to $\times 2$
...
we performed an additional stress-test campaign on the circular trajectory. The already trained optimized PPO policy
...
\begin{table*}[!t]
    ...
    \begin{tabular}{lccccccccc}
        ...
        \midrule
        Nominal optimized PPO & $-0.4300$ & $0.0083$ & $0.2700$ & $0.0220$ & $0$ & $0$ & $0.3510$ & $0.0500$ & $5.3140$ \\
...
\begin{rtwohl}
\noindent\textbf{Robustness results.} The optimized PPO policy remained safe in all stress tests: no collisions
...
or barrier functions~\cite{b22,b24,b23,b34}; and (iv)~hardware-in-the-loop validation on our testbed for In-Space Robotized Services (IROS) with virtually simulated microgravity and predicted energy consumption (see \cref{fig:oos}).
\end{rthreehl}
...
\begin{figure}[!t]
	...
	\end{subfigure}
	\caption{Gradual hybrid testbeds integrate AI-driven digital twins on the left with physical robotic systems on the right, providing a versatile framework for exploring robust policy optimization and domain randomization strategies for sim-to-real generalization.}
	\label{fig:oos}
\end{figure}

Ultimately, our work demonstrates a step in that direction, emphasizing that with the right algorithms
