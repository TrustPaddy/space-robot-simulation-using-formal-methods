...
Simulink/Simscape environment with integrated safety monitoring. Since a  quantitative and qualitative comparison of these methods is currently missing in the on-orbit servicing realm, we benchmark six Reinforcement Learning agents, i.e., TRPO, TD3, SAC, PPO,
...
Building on this result, we quantify the effect of a systematic tuning of neural network architecture and key hyperparameters of PPO for tracking both a circular and ramp-shaped Cartesian path of the end-effector.
...
Indeed, the end-effector of the arm may fail to reach the desired target point if standard motion planning approaches dedicated to robots with a fixed base are simply applied. Further constraints, such as communication latency 
...
The work presented in this paper investigates the use of continuous-action deep RL for the tracking of a desired Cartesian motion of the end-effector of a free-floating space
manipulator. It differs from previous contributions in three ways. First, it focuses on a systematic comparison 
and explores the respective performance of different RL algorithms beyond those mentioned e.g. in \Cref{tab:sample}, thereby quantitatively and qualitatively contributing to filling in a current gap in space robotics. Second,  constraints, such as collision avoidance, are monitored
through formal methods applied from a software-enabled and practical perspective.  Finally, this work takes into 
account the minimization of the base attitude omitted in most recent contributions, as highlighted in the fourth column of \Cref{tab:sample}. We are not aware of any previous work that compares DL algorithms 
...
constrain monitoring through formal methods, to the best of our knowledge (see, also \autoref{tab:sample}). 
...
Recent advances in AI-driven digital twinning have shown considerable benefits for on-orbit servicing. These range from the automated lifecycle 
management of satellites up to  their of true missions. In the project In-Orbit factory \cite{leutert2024ai}, for instance,  digital twins  in 
charge of a contextualized representation of information acquired in real-time from sensors and operations are combined with AI that accommodates 
uncertainties and fosters robustness and generalization. This synergy not only helps bridge the physical and digital world with technical and 
economic merits in terms of accelerated simulation-driven analysis of interdisciplinary robotized approaches under desired boundary conditions 
at low costs. It also enables the development and orchestration \cite{muhlbauer2025towards} of skillful agents likely to autonomously carry 
out complex  assembly, integration, and testing tasks throughout the lifespan of modular satellites  with a workspace penetration and operative 
performance beyond human capabilities \cite{leutert2024ai,muhlbauer2025towards}. Verification and validation is 
another domain in which an adaptation to evolving system designs and operational scenarios is increasingly important and where AI-upskilled 
digital twin excel \cite{maqbool2025systems}. To this end, changes occurring during system design or stemming from operational scenarios are 
described in scenario spaces and integrated with digital twins. This results in an accurate and economic capture and simulation of varied combinations 
of design-scenarios that reveal emerging properties useful for flexible verification and validation in on-orbit servicing \cite{maqbool2025systems}.
...
floating robot (base + arm) at each time step (see~\autoref{fig:spacerobot_slx_robot}). This subsystem takes joint torque inputs and
outputs the resulting states (joint angles, base pose, velocities, etc.) by numerically integrating
the equations of motion \cite{b6}.
...
zero, immediately bringing the system to a safe stop \cite{b22,b23}.
  \item Reference Trajectory Generator: a “Reference” subsystem that provides the desired trajectory
for the robot’s end-effector and (if needed) a desired base attitude (see~\autoref{fig:spacerobot_slx_reference}). In this work, the
...
violation is detected (see  \cref{formal} for details) \cite{b31,b22,b23}.
\end{itemize}
\noindent
...
\begin{figure}[b]
    \centering   
    \includegraphics[width=\linewidth]{Figures/spacerobot_slx_redardfcn.png}
    \caption{Overview of the Reward Subsystem in the Simulink Model \texttt{SpaceRobot.slx} \\
    This figure illustrates the reward and termination logic implemented as a MATLAB function block. The reward is computed 
    from end-effector position and velocity errors, base linear and angular velocities, control effort, and torque smoothness.}
    \label{fig:reward_fcn}
\end{figure}
\noindent
...
penalties for unsafe actions. This reward function is initialized inside a MATLAB function of the Reward Subsystem (see~\autoref{fig:reward_fcn}). The reward is essentially the negative of a weighted cost function that
includes several terms \cite{b7}:
\begin{itemize}
...
towards the desired behavior, while still allowing enough exploration to discover the optimal policy\cite{b7,b2}.
\\ \\
...
% better).
%   \item K4: Mean Base Orientation Error, e.g., average quaternion error magnitude
...
% K7 indicates smoother control inputs which is desirable for mechanical wear and for not exciting
% base motion.
...
%   \item K9: Energy Consumption, approximate average energy expended by the joints ($\int(\tau \cdot \omega) dt$). Lower energy usage is
...
\begin{itemize}
  ...
  \item K9: Energy Consumption, approximate average energy expended by the joints (e.g., $\int(\tau \cdot \dot q) dt$ using an absolute-power proxy). Lower energy usage is better if achieved without sacrificing performance.\\
...
safety components were developed and integrated \cite{b7,b22}.
...
imminent), the system triggers a collision event (see \Cref{fig:spacerobot_slx_colissionmonitor}) \cite{b31}. In our simulation, we set, e.g., $d_{safe} = 5$ cm as the minimum clearance. When triggered, the following occurs: (1) a warning flag is
...
policy output block). This exemplifies how formal methods can complement simulation-based learning: by verifying critical properties on an abstract model of the agent, we gain
...
\end{table}
\noindent
On-policy vs. off-policy reinforcement learning: On-policy algorithms update their policies exclusively using data generated by the current policy itself. 
As a result, the collected experience is always consistent with the policy being optimized, 
which typically leads to stable learning behavior but lower sample efficiency. In contrast, 
off-policy algorithms can reuse experience generated by previous policies or exploratory behavior stored in a replay buffer. 
This improves sample efficiency but often increases sensitivity to hyperparameters and 
instability in highly coupled or safety-critical environment \cite{b2,b7}.
In the context of free-floating space robot control, the strong nonlinear coupling between manipulator motion and base dynamics, 
combined with strict safety constraints, favors the stable policy updates of on-policy methods such as PPO and TRPO, 
whereas off-policy methods may suffer from higher variance and tuning sensitivity.
\\ \\
...
more steps required for convergence. TRPO was moderate ($\approx13$ minutes), faster than SAC/
...
\begin{table}[b]
	...
	\begin{tabular}{lcc}
		...
		Mean Squared EE Tracking Error (K2)      & 0.055  & \textbf{0.0083} \\
...
\end{itemize}
To summarize these improvements,~\autoref{tab:ppo_optimization_kpi} compares the default vs optimized PPO on key metrics:
...
(clipped surrogate objective) made it robust in the face of the highly nonlinear dynamics of the free-floating robot. The integrated safety components (collision avoidance, etc.) effectively acted as a form of
...
While the previous evaluations focused on tracking a smooth circular end-effector (EE) reference, we additionally tested how well the controller generalizes to \emph{linear} trajectories.
This is relevant for practical on-orbit manipulation tasks, where approach and retreat motions are frequently planned as straight-line segments rather than continuous curves.

\noindent
Trajectory definition: the linear reference is constructed from the same circle parameters (center and radius) used for the circular benchmark, but the EE is commanded to move along two straight segments connecting three points on that circle:
...
\end{equation}
The reference is sampled at $T_s=0.01\,\mathrm{s}$, while the PPO policy is updated at $T_{s,\mathrm{agent}}=0.1\,\mathrm{s}$ (same as in the other experiments).
...
\end{figure*}
\noindent
Results: Table~\ref{tab:ppo_linear_kpi} reports the KPI averages over $N_{eval}=30$ evaluation episodes for PPO with the default configuration and the optimized configuration, now tested on the piecewise-linear reference.
The optimized PPO improves overall task success (K1 increases from $-0.4565$ to $-0.2977$), reduces base attitude disturbance (K4 decreases by $\approx 44\%$) (see ~\autoref{fig:r_basis_orientation_compare}), lowers residual base angular velocity (K8 decreases by $\approx 15\%$), and significantly reduces energy consumption (K9 decreases by $\approx 35\%$).
The mean and peak end-effector tracking errors (K2 and K3) remain almost unchanged, with a slight improvement in the mean error (see ~\autoref{fig:r_trajektorie_xy_compare}).
However, the control smoothness metric (K7) becomes worse than the default PPO.
This suggests that the hyperparameter optimization (highly beneficial for circular tracking) does not fully transfer to piecewise-linear motion profiles.
A straightforward mitigation is to include a mixture of reference types (circular, linear segments, and hybrid trajectories) during training (or during hyperparameter search) to explicitly optimize for trajectory generalization.
...
\hline
\end{tabular}
\caption{Generalization test on a piecewise-linear end-effector trajectory (two-segment ramp). Values are averaged over $N_{eval}=30$ episodes. Bold indicates the better value according to the KPI definition (K1 higher is better; K2--K9 lower is better).}
\label{tab:ppo_linear_kpi}
\end{table}

\textbf{Limitations:} Despite the positive outcomes, it is important to recognize the limitations of this work:
\begin{itemize}
...
these could degrade the performance of the learned policy if not accounted for \cite{b8,b22}. The controller
...
info (positions, velocities) was given \cite{b7}.
...
formal analysis and extensive simulation/testing is needed to build trust in the controller \cite{b5,b22}.
...
might improve learning efficiency under safety constraints \cite{b22,b24,b23}.
...
been effective in other robotics applications for transferring policies to real hardware \cite{b8}. We could
...
algorithm to enforce it \cite{b23}. These are active research topics in the RL community (“verificationfriendly RL” and “shielded RL”). Our work provides a concrete case study where these ideas could
be tested: we already have a few formal specs (no collision, no limit violation) that could be
encoded in temporal logic and used in training via, say, a barrier function \cite{b34}.
...
\bibitem{b1}
A. Flores-Abad, O. Ma, K. Pham, and S. Ulrich, ``A review of space robotics technologies for on-orbit servicing,'' 
\emph{Progress in Aerospace Sciences}, vol. 68, pp. 1--26, 2014.
[Online]. Available: \underline{https://doi.org/10.1016/j.paerosci.2014.03.002}

\bibitem{b6}
K. Nanos and E. G. Papadopoulos, ``On the dynamics and control of free-floating space manipulator systems in the presence of angular momentum,'' 
\emph{Frontiers in Robotics and AI}, vol. 4, p. 26, 2017.
[Online]. Available: \underline{https://doi.org/10.3389/frobt.2017.00026}

\bibitem{b11}
E. Papadopoulos, I. Kaliakatsos, and D. Psarros, ``Minimum fuel techniques for a space robot simulator with a reaction wheel and PWM thrusters,'' 
in \emph{Proc. European Control Conf. (ECC)}, 2007, pp. 3391--3398.
[Online]. Available: \underline{https://ieeexplore.ieee.org/document/7068796}

\bibitem{b32}
E. Papadopoulos and A. Abu-Abed, ``Design and motion planning for a zero-reaction manipulator,''
in \emph{Proc. IEEE Int. Conf. Robotics and Automation (ICRA)}, 1994, pp. 1554--1559, doi: 10.1109/ROBOT.1994.351367.
...
\bibitem{wei2025trajectory}Wei, Y., Bai, X. \& Lu, H. Trajectory planning of free-floating space robot for non-cooperative tumbling target capture based on deep reinforcement learning. {\em Robotica}. \textbf{43}, 2674-2692 (2025)
...
\bibitem{b14}
J. Schulman, S. Levine, P. Abbeel, M. Jordan, and P. Moritz, ``Trust region policy optimization,'' 
in \emph{Proc. Int. Conf. Machine Learning (ICML)}, 2015.
[Online]. Available: \underline{https://arxiv.org/abs/1502.05477}
...
[Online]. Available: \underline{https://arxiv.org/abs/1802.09477}

\bibitem{b18}
T. Haarnoja \emph{et al.}, ``Soft actor-critic: Off-policy maximum entropy deep reinforcement learning with a stochastic actor,'' 
in \emph{Proc. Int. Conf. Machine Learning (ICML)}, 2018.
[Online]. Available: \underline{https://arxiv.org/abs/1801.01290}

\bibitem{b15}
J. Schulman, F. Wolski, P. Dhariwal, A. Radford, and O. Klimov, ``Proximal policy optimization algorithms,'' 
arXiv:1707.06347, 2017.
[Online]. Available: \underline{https://arxiv.org/abs/1707.06347}

\bibitem{b21}
R. J. Williams, ``Simple statistical gradient-following algorithms for connectionist reinforcement learning,''
\emph{Machine Learning}, vol. 8, pp. 229--256, 1992, doi: 10.1007/BF00992696.

\bibitem{b7}
R. S. Sutton and A. G. Barto, \emph{Reinforcement Learning: An Introduction}, 2\textsuperscript{nd} ed.
Cambridge, MA, USA: MIT Press, 2018, ch. 3.
[Online]. Available: \underline{http://incompleteideas.net/book/the-book.html}

\bibitem{b22}
J. Garc\'{\i}a and F. Fern\'andez, ``A comprehensive survey on safe reinforcement learning,''
\emph{Journal of Machine Learning Research}, vol. 16, pp. 1437--1480, 2015.

\bibitem{b5}
MathWorks, ``Simulink Design Verifier,'' The MathWorks, Inc., Natick, MA, USA, 2023.
[Online]. Available: \underline{https://www.mathworks.com/help/sldv/index.html}

\bibitem{b23}
M. Alshiekh \emph{et al.}, ``Safe reinforcement learning via shielding,''
in \emph{Proc. AAAI Conf. Artificial Intelligence (AAAI)}, 2018.

\bibitem{b25}
Open Robotics, ``URDF (Unified Robot Description Format),'' ROS~2 Documentation.
[Online]. Available: \url{https://docs.ros.org/en/rolling/Tutorials/Intermediate/URDF/URDF-Main.html}

\bibitem{b26}
MathWorks, ``Import URDF Models,'' MATLAB \& Simulink Documentation.
[Online]. Available: \url{https://www.mathworks.com/help/sm/ug/urdf-import.html}

\bibitem{b27}
MathWorks, ``smimport -- Import a CAD, URDF, or Robotics System Toolbox model,'' MATLAB \& Simulink Documentation.
[Online]. Available: \url{https://www.mathworks.com/help/sm/ref/smimport.html}

\bibitem{b28}
MathWorks, ``Mechanism Configuration -- Mechanism-wide gravity and simulation parameters,'' MATLAB \& Simulink Documentation.
[Online]. Available: \url{https://www.mathworks.com/help/sm/ref/mechanismconfiguration.html}
...
\bibitem{b9}
J. Virgili-Llop, ``SPART: Spacecraft Robotics Toolkit,'' Open-source software, 2017.
[Online]. Available: \underline{https://github.com/NPS-SRL/SPART}

\bibitem{b12}
E. Ribeiro, R. Mendes, M. Terra, and V. Grassi, ``Dynamic parameter identification of a 7-DoF lightweight robot manipulator using probabilistic differential optimization,'' 
in \emph{Proc. IEEE Int. Conf. Systems, Man, and Cybernetics (SMC)}, 2022, pp. 1917--1923.
[Online]. Available: \underline{https://ieeexplore.ieee.org}

\bibitem{b3}
MathWorks, ``6-DOF Joint (Simscape Multibody),'' The MathWorks, Inc., Natick, MA, USA, 2023.
[Online]. Available: \underline{https://www.mathworks.com/help/sm/ref/6dofjoint.html}

\bibitem{b34}
A. D. Ames \emph{et al.}, ``Control barrier functions: theory and applications,''
in \emph{Proc. European Control Conf. (ECC)}, 2019. (arXiv:1903.11199)

\bibitem{b31}
MathWorks, ``Spatial Contact Force -- Model spatial contact between two geometries,'' MATLAB \& Simulink Documentation.
[Online]. Available: \url{https://www.mathworks.com/help/sm/ref/spatialcontactforce.html}

\bibitem{b29}
MathWorks, ``Solver Configuration -- Physical network environment and solver configuration,'' MATLAB \& Simulink Documentation.
[Online]. Available: \url{https://www.mathworks.com/help/simscape/ref/solverconfiguration.html}

\bibitem{b30}
MathWorks, ``Fixed-step size (fundamental sample time),'' MATLAB \& Simulink Documentation.
[Online]. Available: \url{https://www.mathworks.com/help/simulink/gui/fixedstepsizefundamentalsampletime.html}

\bibitem{b2}
J. Kober, J. A. Bagnell, and J. Peters, ``Reinforcement learning in robotics: A survey,'' 
\emph{The International Journal of Robotics Research}, vol. 32, no. 11, pp. 1238--1274, 2013.
[Online]. Available: \underline{https://doi.org/10.1177/0278364913495721}

\bibitem{b16}
T. P. Lillicrap \emph{et al.}, ``Continuous control with deep reinforcement learning,'' 
arXiv:1509.02971, 2015.
[Online]. Available: \underline{https://arxiv.org/abs/1509.02971}

\bibitem{b4}
MathWorks, ``Reinforcement Learning Toolbox -- rlPPOAgent,'' The MathWorks, Inc., Natick, MA, USA, 2023.
[Online]. Available: \underline{https://www.mathworks.com/help/reinforcement-learning/ref/rl.agent.rlppoagent.html}

\bibitem{b8}
J. Tobin \emph{et al.}, ``Domain randomization for transferring deep neural networks from simulation to the real world,'' 
in \emph{Proc. IEEE/RSJ Int. Conf. Intelligent Robots and Systems (IROS)}, 2017, pp. 23--30.
[Online]. Available: \underline{https://doi.org/10.1109/IROS.2017.8202133}

\bibitem{b24}
J. Achiam, D. Held, A. Tamar, and P. Abbeel, ``Constrained policy optimization,''
in \emph{Proc. Int. Conf. Machine Learning (ICML)}, 2017.
