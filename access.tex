...
The optimized PPO improves overall task success (K1 increases from $-0.4464$ to $-0.3653$), reduces base attitude disturbance (K4 decreases by $\approx 35\%$) (see Fig.~\ref{fig:r_basis_orientation_compare}), lowers residual base angular velocity (K8 decreases by $\approx 29\%$), and significantly reduces energy consumption (K9 decreases by $\approx 31\%$).
However, the mean and peak tracking errors (K2 and K3) slightly increase on this linear trajectory (see~\autoref{fig:r_trajektorie_xy_compare}), and the control smoothness metric (K7) becomes worse than the default PPO.
...
\textbf{Metric (KPI)} & \textbf{PPO (Default)} & \textbf{PPO (Optimized)} \\
\hline
Mean Episodic Reward (K1)            & -0.4464 & \textbf{-0.3653} \\
Mean Squared EE Position Error (K2)  & \textbf{0.0072} & 0.0085 \\
Max EE Position Error (K3)           & \textbf{0.1347} & 0.1479 \\
Mean Base Orientation Error (K4)     & 0.0440 & \textbf{0.0284} \\
Collision Rate (K5)                 & 0 & 0 \\
Joint Limit Violation Rate (K6)     & 0 & 0 \\
Control Smoothness (K7, jerk)       & \textbf{0.5711} & 0.6684 \\
Mean Base Angular Velocity (K8)      & 0.0459 & \textbf{0.0324} \\
Energy Consumption (K9)              & 7.8367 & \textbf{5.3739} \\
\hline
\end{tabular}
