...
\begin{abstract}
Free-floating space robots exhibit a strong dynamic coupling between
...
\begin{itemize}
  ...
  \item  \textit{Integration of formal-methods-based safety monitoring.}
  Formal-methods tooling is integrated into the learning loop \cite{b5, b6, b22, b23}. A
 ...
 shaping allows the agent to learn the task while penalizing base disturbance and
 violations of the monitored constraints \cite{b25,b26,b27,b28,b7}.
 \item \textit{Optimization of the PPO agent.} The default PPO agent converged in only three
 of ten runs, but its successful runs tracked the path with an error similar to that of TRPO. PPO is
 therefore tuned to reduce this seed dependence. Hyperparameters such as the learning rates,
 the rollout horizon, the mini-batch size, and the entropy weight are adjusted. The hidden layers of the
 default networks are kept. The tuned agent completes all ten runs without an
...
configuration.The remainder of this work is organized as follows. \cref{sec:sim}
...
\cref{fig:spacerobot_slx_colissionmonitor}) \cite{b31}. If $d_{\min}$ falls below the safety
...
the penalty in \eqref{eq:rfail}. The monitor was tested by intentionally commanding the arm
toward the base and by starting an episode in a colliding configuration. In both cases, the contact was
detected, and the robot was stopped (see~\cref{fig:collision_combined}). During training,
...
formal stability margins, are not verified. In particular, the runtime monitors detect
...
would have violated a monitored property and penalized the corresponding actions. For TRPO
and the optimized PPO agent, no violations of the monitored constraints were observed in any evaluation
episode \cite{b23}. The other agents violated a monitored constraint in part of their runs, and
the monitors terminated these episodes. This is empirical evidence for the tested initial
...
\begin{table*}[!t]
\centering
\caption{Performance of the successful runs of the default agents. All values are the mean and the
...
\textbf{Performance of the Successful Runs.} \cref{tab:kpi_comp} summarizes the tracking error $K_2$,
...
and 0.066 (\cref{fig:boxplot_k2}). The median of TRPO is 0.0061.
...
Reinforcement Learning Toolbox \cite{b4}. The tuning concerns the hyperparameters, while the
...
manipulation task in the simulated space environment under the stated assumptions. With the
...
it trained faster. No violations of the monitored safety properties were recorded for
...
PPO did not prevent the seed dependence observed in \cref{comparison}. The runtime
...
Combined with the penalty-driven reward, this
...
mission scenarios. It was selected in an earlier grid search with a single run per
candidate. The other algorithms were compared only with their toolbox default hyperparameters, and tuning
them might change their ranking. Third, the \emph{evaluation scope} is limited.
...
partial observability. Fifth, the \emph{safety monitoring} covers selected
...
with the toolbox default hyperparameters and ten seeds each. For the considered
simulated task and evaluation setup, TRPO was the only default agent that completed
...
significant. Runtime monitoring of selected safety properties in the training loop, together
with an offline torque-bound check, detected and terminated the violations of the
other agents. These simulation results, obtained under the stated idealizing
