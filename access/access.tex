...
supervised learning are scarce, which makes simulation-driven policy training through interaction and task-specific rewards attractive. This work therefore benchmarks six deep reinforcement learning (RL)
...
configuration is trained with ten seeds. Success rate and performance of successful runs are
evaluated separately with unified metrics. With default hyperparameters, only TRPO completes all ten
runs without a safety abort caused by a monitored collision or violation of joint limit, whereas the other agents terminate 40\% to 80\% of their evaluation
...
TRPO, but not significantly after a multiple-comparison correction. Both agents complete all runs under observation noise, reduced torque limits, an external joint torque, and masses, inertias, and joint damping coefficients uniformly increased by 50\%. All results are obtained in simulation under controlled assumptions and do
not establish operational readiness. The safety component comprises runtime monitors for selected
properties and a formal check of torque saturation, but does not constitute a formal proof of closed-loop safety.
\end{abstract}
\begin{keywords}
...
\begin{figure}[!t]
	...
	to the physical setup (right). This work covers only the simulation
...
\begin{itemize}
 ...
 to reduce this seed dependence. The optimized PPO agent is therefore used to assess whether conservative update settings can reduce the dependence of PPO on seed. Hyperparameters such as the learning rates, the rollout horizon, the
...
\cite{leutert2024ai,muhlbauer2025towards,kaigom2020value,kaigom2023metarobotics}. DTs combine contextualized real-time sensor data with AI methods that
...
monitored safety properties. The main observation is that no RL algorithm is universally superior. The benchmark indicates that bounding the size of policy updates is important when Cartesian tracking, stabilization of the base attitude, and constraint satisfaction are coupled in a free-floating robot system.

The results show that an RL-based controller can perform the desired
manipulation task in the simulated space environment under the considered assumptions. With the
...
Third, the \emph{evaluation scope} is limited. Each
...
conditional on the tested setup. The comparison should therefore be understood primarily as evidence of training robustness that depends on seed under the tested setup. It is not a definitive ranking of all considered RL algorithms. 

Fourth, the \emph{state representation} was manually designed. Learned
...
\bibitem{la2025deep}La, T. \& Kaigom, E. Deep learning for model-free prediction of thermal states of robot joint motors. {\em IFAC-PapersOnLine}. 211-216 (2025)

\bibitem{kaigom2020value}Kaigom, E. \& Roßmann, J. Value-driven robotic digital twins in cyber–physical applications. {\em IEEE Transactions On Industrial Informatics}. 3609-3619 (2020)

\bibitem{kaigom2023metarobotics}Kaigom, E. Metarobotics for industry and society: Vision, technologies, and opportunities. {\em IEEE Transactions On Industrial Informatics}.  5725-5736 (2023)
...
Sciences, Germany. He serves the academic community as an Associate Editor. His current research interests include robotics, digital twins, agentic systems, and artificial intelligence $\slash$ machine
learning. 
\end{IEEEbiography}
