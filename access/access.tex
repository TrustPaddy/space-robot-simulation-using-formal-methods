...
safety monitoring, space robotics,  trajectory tracking
\end{keywords}
...
\begin{table*}[!t]
  ...
  EE\,=\,end-effector; Formal Meth.\,=\,formal-methods-based safety components; Base Dist.\,Min.\,=\,base
  disturbance minimization; $d_{\mathrm{safe}}$\,=\,minimum admissible distance; $\tau_{\max}$\,=\,torque
  bound; $K_1$--$K_9$\,=\,evaluation KPIs (see \cref{RL}); N/R\,=\,not reported; $^{\dagger}$runtime
  monitors for selected properties and an offline torque-bound check on a simplified model, not a formal
  proof of closed-loop safety.}
	\label{tab:sample}
	\scriptsize
	...
	\begin{tabular}{lcccccccc}
		...
		\midrule
		\textbf{Ours} & \makecell{\textbf{PG, PPO,}\\\textbf{TRPO, DDPG,}\\\textbf{TD3, SAC}} &
...
algorithms for free-floating space robot control. In addition, the surveyed works do not combine three aspects.
These are (i)~a multi-algorithm benchmark under unified conditions, (ii)~explicit minimization of the base
attitude disturbance alongside end-effector tracking, and (iii)~formal-methods-based runtime safety monitoring in the
RL training and evaluation loop. This work addresses these aspects together in a reproducible simulation benchmark
with runtime monitoring of selected safety properties, which extends the individual studies listed in
\cref{tab:sample}.
...
(PPO) \cite{b15}, vanilla Policy Gradient (PG) \cite{b21}, and Deep Deterministic Policy Gradient (DDPG) \cite{b16}.
The comparison is performed under unified conditions and examines which algorithm offers the most
favorable trade-off between tracking accuracy, base disturbance, and training stability.
  \item \textit{Integration of formal-methods-based safety monitoring.} Formal-methods tooling is
  integrated into the learning loop \cite{b5, b6, b22, b23}. A collision monitor, a joint-limit monitor,
  and a momentum-conservation check run at runtime, and the joint-torque bound is additionally checked
  offline with Simulink Design Verifier on a simplified abstraction of the policy. This combination
  monitors selected safety properties (collision events, joint limits, and bounded actuator commands)
  during training and testing. It does not constitute a formal proof of closed-loop stability, task
  completion, or collision avoidance for the full nonlinear multibody model.
 ...
 shaping allows the agent to learn the task while penalizing base disturbance and violations of the
 monitored constraints \cite{b25,b26,b27,b28,b7}.
 \item \textit{Optimization of the PPO agent.} Based on the comparative results, PPO is selected for
 further tuning. Hyperparameters such as the learning rates, the rollout horizon, the mini-batch size,
 and the entropy weight are adjusted to improve training stability and policy performance. The hidden
 layers of the default networks are kept. Relative to the default configuration, the optimized PPO agent
 reduces the tracking error and produces smoother control inputs on the tested KPIs. No violations of the
 monitored safety properties are observed in the 30 evaluation episodes.
\end{itemize}

Within the considered simulation setup, the combination of multibody simulation, deep RL, and runtime
safety monitoring yields control policies without observed violations of the monitored constraints in the
evaluation episodes. The optimized policy also reduces the tracking error and the base disturbance
relative to the default configuration. The remainder of this work is organized as follows. \cref{sec:sim}
describes the system model and the simulation environment. \cref{RL} introduces the reinforcement
learning framework. \cref{formal} presents the safety monitors and formal-methods components.
...
free-floating space robots. Existing works seldom integrate formal-methods-based safety monitoring into the RL
...
multi-criteria benchmarking, base disturbance minimization, and formal-methods-based safety monitoring.
...
contributions (see \cref{tab:sample}). Third, the integration of formal-methods-based safety monitoring into the RL
training and evaluation loop has not been reported for this class of systems in the surveyed works. This work
addresses the three gaps together within a controlled simulation benchmark. It benchmarks six RL algorithms under
unified conditions and treats base attitude minimization as an explicit control objective. Runtime monitoring of
selected safety properties is integrated into the RL training and evaluation loop. The actuator-torque bound is
additionally checked offline on a simplified verification model.
...
used as training and validation environments for learned control strategies. During training, the initial
joint configuration of each episode is sampled from a bounded uniform distribution. The agent therefore
learns from a range of admissible initial conditions instead of a single fixed start.
...
monitor reports a contact, the switch overrides the torques to zero~\cite{b22,b23}. The \emph{Reference
...
contact or a joint-limit violation terminates the episode~\cite{b31,b22,b23}. Details are given in
\cref{formal}.
...
around $(4.0,\,0,\,0)\,\mathrm{m}$ within $T = 8.5\,\mathrm{s}$ (see~\cref{fig:ref_kreis_gesamt}). Before
training, the trajectory was checked with inverse kinematics for geometric feasibility, that is, for
compliance with the joint limits and the absence of singularities. The inverse-kinematics solution gave a
consistent set of joint trajectories that realize the desired end-effector motion. The path is therefore
kinematically feasible. A failure of the RL agent can thus be attributed to learning and not to a
kinematically infeasible reference. During simulation, the reference subsystem outputs the desired
end-effector position and velocity as time series $EE_{\mathrm{ref}}(t)$ and $EE_{v,\mathrm{ref}}(t)$ at
each time step. The instantaneous position and velocity tracking errors are derived from these signals.
They enter both the observation vector and the reward computation that penalizes large errors.
...
value of $\tau_{\max}$ is selected according to the robot design ($\tau_{\max} = 2\,\mathrm{N\,m}$ in
this work), provided it is large enough to follow the reference trajectory. The saturation enforces both
the actuator limits and an upper bound on the applied torque for safety reasons. The action space is
therefore $\mathbb{R}^4$, which is compatible with policy gradient and actor-critic methods.
...
states, and logging signals are reset as well \cite{b7}.
...
up to the termination step. The KPIs are first computed for each seed over the 30 random episodes.
The seed dependence and the performance reached are then reported separately. A training run
counts as successful when none of its 31 evaluation episodes is terminated by a monitor. The first
table reports the number of successful runs per configuration and the share of evaluation episodes
that a monitor terminated, taken over all runs. The KPI tables report the mean and the standard
deviation over the successful runs only, together with their number. These KPI values are therefore
conditional on a successful run. Algorithms are compared on the seed level with two-sided
Mann--Whitney $U$ tests over the successful runs and a Holm correction for multiple comparisons. A
test is reported only when both configurations have at least four successful runs. The shares of
successful runs are compared with Fisher's exact test, also with a Holm correction.
...
of the episode reward over the last 100 training episodes, an indicator of training stability. $T_3$ is
...
velocities, from which $p_{\mathrm{tot}} = \sum_{i} m_{i} v_{i}$ is computed. In an ideal simulation,
$p_{\mathrm{tot}}$ remains close to zero throughout an episode. Small numerical deviations on the order
of $10^{-6}\,\mathrm{N\,s}$ may arise from time-integration errors. A persistent, systematic drift of
$p_{\mathrm{tot}}$ would indicate an unintended external force or a modeling error, such as an unmodeled
constraint. In the reported experiments, $p_{\mathrm{tot}}$ remained within $10^{-6}\,\mathrm{N\,s}$ of
zero throughout each episode with the learned policy (see~\cref{fig:impulserhaltung_scope}). This
indicates that the simulation conserves momentum within the numerical tolerance. The momentum observer is
a diagnostic tool. It is evaluated offline and does not influence the training.
...
$d_{\min}$ between non-adjacent robot bodies from the URDF collision geometry (see
\cref{fig:spacerobot_slx_colissionmonitor}) \cite{b31}. If $d_{\min}$ falls below the safety threshold
$d_{\mathrm{safe}} = 2\,\mathrm{cm}$, a warning is raised and logged. If a contact between two bodies is
detected, all commanded joint torques are overridden to zero and the episode is terminated with the
penalty in \eqref{eq:rfail}. The monitor was tested by intentionally commanding the arm toward the base
and by starting an episode in a colliding configuration. In both cases, the contact was detected, and the
robot was stopped (see~\cref{fig:collision_combined}). During training, collisions occurred in early
exploratory episodes for some agents, but were captured by the monitor and penalized through episode
termination. In the final evaluation, no collisions were observed for any agent ($K_5 = 0$ over all 30
evaluation episodes).
...
penalty forces, i.e., a stiff barrier that opposes further motion beyond the limit. Second, a joint-limit
monitor in the reward block compares every joint angle with its admissible range at each agent step. A
violation terminates the episode with the penalty in \eqref{eq:rfail}, which prevents further motion into
the limit. After training, the joint-limit violation rate is low. PPO and TRPO reach the limits in a
few percent of evaluation steps (5.1\% and 2.1\%, respectively), whereas TD3, SAC, PG, and DDPG remain
within the admissible range ($K_6 \approx 0$). The higher $K_6$ value of PPO ($K_6 = 0.0505$) coincides
with the lowest tracking error and may indicate a more extensive use of the workspace. In all cases,
the enforcement mechanism stopped further simulated motion at the limit. The torques were set to zero in
the offending step and the episode was marked as failed. For the optimized PPO agent
(\cref{sec:ppo_opt}), no such events are observed in the evaluation episodes ($K_6 = 0$).
...
formal stability margins, are not verified. In particular, the runtime monitors detect violations and
terminate the episode but do not prove their absence, and the offline SLDV check establishes only the
torque bound on the simplified verification model. Neither closed-loop stability, task completion, nor
collision avoidance over all possible trajectories of the full nonlinear Simscape model is formally
verified. A complete formal specification of the mission, for example temporal-logic specifications such
as ``the robot shall accomplish $X$ without $Y$ happening'', would be considerably more complex and is
beyond the scope of this work. This work targets the safety properties that are considered most relevant
for the task. Within this scope, the runtime monitors terminated episodes that would have violated a
monitored property and penalized the corresponding actions. By the end of training, no violations of the
monitored constraints were observed in the reported evaluation episodes \cite{b23}. This is empirical
evidence for the tested initial configuration and reference trajectories, not a guarantee over all
reachable states.
...
are reported as metric $T_3$ in \cref{tab:kpi_comp}. They range from about 7 minutes (PG) to 35 minutes
...
\begin{figure}[!b]
  ...
  counterexample for any analyzed execution path of the simplified verification model. The result does not cover
  other properties or the full nonlinear Simscape model.}
...
networks with two hidden layers of 128 ReLU units. $\alpha_a$ and $\alpha_c$ denote the actor and critic
learning rates.}
...
\textbf{Performance Metrics.} The agents were evaluated on the KPIs introduced in \cref{RL} with the
protocol described there, which uses 30 random initial states for each of the ten seeds.
...
PPO attains the lowest mean squared tracking error ($K_2$), the lowest base orientation error ($K_4$), and the
lowest jerk ($K_7$), with the shortest training time ($T_3$). DDPG attains the lowest maximum error ($K_3$). TRPO
is close to PPO on several metrics but requires more training time. The off-policy methods (TD3, SAC, DDPG) show
larger errors or a larger late-training reward variance, for example a high $T_1$ for DDPG. Values are averaged over
...
\begin{itemize}
        \item PPO shows the most favorable combination of training stability (low $T_1$), training time ($T_3$),
        tracking accuracy (low $K_2$), and control smoothness (low $K_7$) in this comparison, at the cost of a
...
Overall, the tests are consistent with a favorable balance of PPO across the KPIs in this comparison. The smoother
control signals of PPO are significant against all other agents, and its base orientation error does not differ
significantly from that of TRPO.
\cref{fig:boxplots} shows the distribution of four selected KPIs over the 30 evaluation episodes as boxplots,
including the spread and the outliers of each agent.
\cref{fig:ci_plots} shows the corresponding 95\% bootstrap confidence intervals for each agent and KPI.
...
PPO is selected for further tuning. The PPO agent used so far corresponds to the default configuration of
MATLAB's Reinforcement Learning Toolbox \cite{b4}. The modifications aim at faster convergence, lower
tracking and base orientation errors, and the avoidance of the remaining joint-limit violations. The
tuning concerns the hyperparameters, while the hidden layers of the networks are kept.
...
\textbf{Hyperparameter Tuning.} The configuration was determined in a preliminary coordinate-wise grid
search and is kept fixed for all experiments reported here. Each hyperparameter was swept over a discrete
set of candidate values while all others were held at their current best values. The configuration with
the highest mean episodic reward over the final 50 of the $N_{\mathrm{train}}=1000$ training episodes was
selected. In total, 16 training runs of 1000 episodes each were conducted during this tuning phase. The
swept parameters, their candidate values, and the selected values are as follows:
\begin{itemize}
  \item \textit{Actor learning rate.} Candidates $\{1\!\times\!10^{-3},\;5\!\times\!10^{-4}\}$, selected
  $1\!\times\!10^{-3}$. Both candidates are an order of magnitude below the toolbox default of $10^{-2}$,
  which reduces the size of the policy updates.
  \item \textit{Critic learning rate.} Candidates $\{1\!\times\!10^{-3},\;5\!\times\!10^{-4}\}$, selected
  $5\!\times\!10^{-4}$, which gave the highest mean episodic reward in the search.
  ...
  \item \textit{Fixed settings.} The discount factor was set to $\gamma = 0.995$, the number of epochs
  per update to 10, and the gradient threshold of both optimizers to 1. The clip factor of 0.2
  and the GAE factor of 0.95 remain at their default values.
\end{itemize}

In the tuning runs, the resulting configuration gave a more stable and better-performing PPO agent.
Compared with the default configuration, the optimized agent uses learning rates that are ten to twenty
times smaller, larger rollouts and mini-batches, more epochs per update, and gradient clipping. Together,
these settings limit the size of each policy update.
...
smoothness, and energy consumption together, without observed violations of the monitored constraints. These
results support the use of hyperparameter tuning for space-robot control under the considered conditions.
...
orientation error $K_4$ rises by more than an order of magnitude (from $0.022$ to $0.668$). The
end-effector tracking error ($K_2 = 0.017$) remains lower than in all other ablation variants. This
...
\textbf{Note on the Energy Metric $\boldsymbol{K_9}$.} $K_9$ is lower in every ablation variant than in the
baseline ($5.314$). This does not indicate a higher energy efficiency. It is likely an artifact of earlier episode
termination when the agent fails (for example, after a joint-limit violation or a contact), which truncates the
integration window. $K_9$ should therefore only be interpreted together with $K_1$, $K_2$, and $K_4$.
...
The ablation study in~\cref{sec:reward_ablation} identifies which reward components are
most influential. A complementary question is how sensitive the results are to a \emph{moderate} miscalibration of
the reward weights. The analysis examines whether the PPO agent still converges to a working policy when the cost
weights are perturbed and whether the ranking of the algorithms in \cref{tab:kpi_comp} is likely to change. A
one-at-a-time sensitivity analysis was performed on four cost weights. These are the two dominant weights $W_p$ and
$W_{\mathrm{ori}}$ from the ablation and the two secondary weights $W_v$ and $W_{\omega_b}$. Each weight was
multiplied by 2 and by 0.5 around its baseline value ($W_p=150$, $W_v=25$, $W_{\mathrm{ori}}=200$,
$W_{\omega_b}=8$). All other settings were kept at the baseline. These are the remaining reward weights, the
optimized PPO hyperparameters (\cref{sec:ppo_opt}), the random seeds, the episode horizon, and the evaluation
protocol with $N_{\mathrm{eval}}=30$. This yields eight retrained policies, whose KPIs are summarized
in~\cref{tab:reward_sensitivity}.

\begin{table}[!t]
    \centering
    \caption{One-at-a-time sensitivity analysis of four reward weights. Each row corresponds to
...
\textbf{Safety Metrics.} Across all eight perturbed configurations, no collisions were observed ($K_5=0$), and the
joint-limit violation rate stayed below $0.53\%$ ($K_6 \le 0.0053$). In the tested cases, a miscalibration of the
reward weights mainly degraded the task performance. The runtime monitors of \cref{formal} terminate episodes with
violations independently of the reward weights.
...
2 or 0.5 is therefore unlikely to change the ranking in \cref{comparison}. This argument is indirect,
because only the PPO agent was retrained with perturbed weights. A full cross-evaluation of all six
algorithms over the eight configurations would multiply the training budget by six and is beyond the
...
\textbf{Robustness Results.} No violations of the monitored properties occurred in any stress test. No collisions
or joint-limit violations were observed in any condition ($K_5=K_6=0$). The strongest degradation is observed for
the combined stress case and for parameter uncertainty. This is plausible, because both change the effective
dynamics seen by the fixed policy. In the combined case, no divergence and no monitor-triggered termination were
observed. The mean base orientation error increased to $K_4=0.0447$ and the mean tracking error to $K_2=0.0078$.
Across all non-ideal conditions, $K_7$ increases compared with the nominal optimized PPO result, which suggests
that the policy compensates for the perturbations with less smooth torque profiles. The lower $K_9$ values under
stress should not be interpreted as efficiency gains. They likely reflect the changed actuator limits, the modified
dynamics, and the injected disturbances, which alter the mechanical-work calculation. Overall, the experiment
indicates that the performance of the optimized PPO policy degrades gradually under sensor noise, disturbances,
parameter uncertainty, and reduced actuator authority, with no violations of the monitored constraints in the
tested episodes. Each perturbation is tested at a single level and from a fixed initial configuration. These stress
tests therefore do not replace a systematic robustness characterization.
...
Both evaluations support the use of hyperparameter tuning for space-robot control under the considered conditions.
The piecewise-linear results also indicate that trajectory-specific tuning may be needed to maintain control
smoothness across reference geometries.
...
approximately halved ($K_8$, $0.095\,\mathrm{rad/s} \to 0.050\,\mathrm{rad/s}$). No violations of the
monitored safety properties were recorded over the 30 evaluation episodes ($K_5 = K_6 = 0$), compared
with a $5.1\%$ joint-limit violation rate for the default configuration. Control smoothness improves by
$48.5\%$ ($K_7$), and the energy consumption decreases by $16.0\%$ ($K_9$). These effects are observed
together in the reported evaluation rather than as an apparent trade-off between the selected objectives
\cite{b1,b7}.
...
under the nonlinear free-floating dynamics. The runtime monitors (collision monitor and joint-limit monitor)
supervise the training. They terminate episodes in which the
policy would violate a monitored constraint and apply a terminal penalty \cite{b15,b23,b22}. Combined with
the penalty-driven reward, this led to a final policy that did not violate the monitored constraints
in the reported evaluation. The present simulation study does not establish that this behavior transfers to a
physical system. It suggests only that safety checks embedded in the training simulator may remain useful as
runtime watchdogs, provided that the deployed system stays within the simulator's fidelity range, which remains
to be validated \cite{b22,b24}.
...
mission scenarios. Third, the \emph{evaluation scope} is limited. Each algorithm was trained in three
runs and evaluated over 30 episodes from a fixed initial configuration, for a single robot model and a
small set of reference trajectories. This does not fully characterize seed sensitivity, generalization
across initial states, target trajectories, and robot parameters, or robustness to disturbances. The
reported ranking and improvements are therefore conditional on the tested setup. Fourth, the \emph{state
representation} was manually designed. Learned representations or recurrent architectures (LSTM) might
improve performance under partial observability. Fifth, the \emph{safety monitoring} covers selected
...
with the toolbox default hyperparameters. For the considered simulated task and evaluation setup, PPO
showed the most favorable trade-off between tracking accuracy, base disturbance, and training stability.
Subsequent hyperparameter tuning of the PPO agent yielded a policy that reduces the tracking error by
$67.7\%$ relative to the default configuration. It also reduces the base orientation error by a factor of
approximately three and the energy consumption by $16.0\%$, and no joint-limit violations or collisions
were observed in the 30 evaluation episodes. Runtime monitoring of selected safety properties in the
training loop, together with an offline torque-bound check, helped to keep these results within the
monitored constraints in the evaluated episodes. This addresses a part of the black-box nature of learned
controllers. These simulation results, obtained under the stated idealizing assumptions, do not establish
...
for language editing and grammar enhancement. Anthropic Claude~\cite{anthropic_claude} was used as a
coding assistant for developing and debugging the simulation and evaluation scripts. All experiments,
results, and conclusions were designed, checked, and verified by the authors.
