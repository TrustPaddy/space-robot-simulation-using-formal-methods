...
\title{\hlone{[R2.5] Benchmarking Deep Reinforcement Learning With Runtime Safety Monitoring for Cartesian Path Tracking of Free-Floating Space Robot Manipulators}}
...
Frankfurt am Main, Germany \\(e-mail: patrick.ermisch@proton.me, kaigom@fra-uas.de)}
...
\markboth
{P. S. Ermisch and E. G. Kaigom: Benchmarking Deep RL With Runtime Safety Monitoring for Free-Floating Space Manipulators}
{P. S. Ermisch and E. G. Kaigom: Benchmarking Deep RL With Runtime Safety Monitoring for Free-Floating Space Manipulators}

\corresp{Corresponding author: Eric Guiffo Kaigom (e-mail: kaigom@fra-uas.de).}

\begin{abstract}
\hlone{[R2.1, R2.2, R2.3] The base reaction of free-floating space robots
...
planar Cartesian path tracking of an FFSR in a physics-based Simulink/Simscape environment with runtime
...
readiness. The safety component comprises runtime monitors for selected properties and a formal check
of torque saturation, but is not a formal proof of closed-loop safety.}
\end{abstract}
\begin{keywords}
Free-floating on-orbit servicing, Proximal Policy Optimization (PPO), reinforcement learning,
\hlone{safety monitoring}, space robotics, trajectory tracking, extended agentic systems
intelligence (EASI)
\end{keywords}
...
\begin{figure}[!t]
	...
	\end{subfigure}
	\caption{Concept of a hybrid testbed for in-space robotic services. An AI-driven digital
...
\cite{kaigom2025deep,la2025deep}, as illustrated in \cref{fig:oos}. Unlike standard DTs that
primarily mirror sensed
real-time data from physical counterparts, AI-enabled DTs combine physics-based simulation with
learning components to contextualize system states and evaluate representative operating conditions.
Such conditions can include uncertain dynamics, disturbances, faults, and contact-rich
...
flight data are limited and the constraints are safety-critical. The DT in this work covers
only the space robot under nominal free-floating conditions (see \cref{sec:sim}).
...
\begin{table*}[!t]
  ...
  EE\,=\,end-effector; \hlone{[R2.1]} Formal Meth.\,=\,formal verification of safety properties;
  ...
  limits are monitored at runtime, and the closed loop is not formally verified.}}
	...
	\begin{tabular}{lcccccccc}
		...
		\midrule
		\textbf{This work} &
		  \makecell{\textbf{PG, PPO,}\\\textbf{TRPO, DDPG,}\\\textbf{TD3, SAC}} &
...
The works listed in \cref{tab:sample} do not provide such a comparison. Second, collisions
and joint limits are monitored at runtime in the training and evaluation loop, and the torque bound of the
actuation path is checked formally (see \cref{formal}). Third, base attitude minimization during
...
free-floating space robots that jointly considers base attitude minimization and runtime
...
\begin{itemize}
  ...
  \cite{b21}, and Deep Deterministic Policy Gradient (DDPG)~\cite{b16}. \hlone{[R2.3] The
  ...
  \item \hlone{[R2.1]} \textit{\hlone{Runtime safety monitoring with a formal check of the
 ...
 penalizing base disturbance and violations of the monitored constraints
 \cite{b7}.
 ...
 therefore tuned} to assess whether more conservative update settings reduce this seed
 dependence. Hyperparameters such as the learning rates,
 the rollout horizon, the mini-batch size, and the entropy weight are adjusted. The hidden layers of the
 default networks are kept, but the mean output of the policy is not bounded by a tanh
 layer. \hlone{[R2.3, R2.5] The tuned agent completes all ten runs without an observed violation of
...
reinforcement learning framework. \cref{formal} presents the safety monitors and the formal
...
free-floating space robots. Existing works seldom integrate runtime safety monitoring or formal
...
multi-criteria benchmarking, base disturbance minimization, and runtime safety
...
operations, is omitted in most RL-based contributions (see \cref{tab:sample}). \hlone{[R2.1]}
...
and evaluation loop. The torque bound of the actuation path is additionally checked
formally.
...
\cite{leutert2024ai,muhlbauer2025towards,kaigom2020value,kaigom2023metarobotics}. DTs
...
In this work, the DT covers only the chaser space robot. A target satellite and the contact
with it are not modeled. The DT is based on an information model that consists of the URDF description of
the robot and the parameters in \cref{tab:robot_params}. It describes the kinematics, the
...
control strategies. During training, the initial joint angles of each episode are drawn
uniformly within $\pm 1^\circ$ around the nominal configuration (see \cref{RL}). This small
...
\begin{figure*}[!t]
  ...
  an RL agent block that outputs joint torques. The same model is used for all six algorithms.
  The reward and termination logic evaluate end-effector tracking, base motion, and control effort.
  Two further blocks add the observation noise and the external joint torque of the stress
...
The geometry and the inertial parameters of the base and the links are listed in
\cref{tab:robot_params}. The same values are used in the URDF file and in the Simscape model.
...
Simscape Multibody physics engine. Gravity is set to $[0~0~0]^T$ in the mechanism
...
angular momentum are conserved, and arm motions induce a reactive motion of the base
...
continuous control~\cite{korber2021comparing}. However, it has no native interface to
...
well suited for hardware-in-the-loop (HIL) development~\cite{collins2021review}. It offers no
native path to formal verification tools. Simscape Multibody provides a multibody solver for the
free-floating base--manipulator coupling. It runs in the same Simulink model as the runtime
...
the RL agent, and the safety checks.
...
joint torques~\cite{b6}. The \emph{Controller} subsystem executes the learned RL policy. The commanded joint torques pass through a saturation block and a switch before reaching the plant.
When the collision monitor reports a contact, the switch overrides the torques to zero.
...
in \cref{RL}). Finally, two \emph{safety monitors} check the operational constraints at
...
detected contact or a joint-limit violation terminates the episode. Details are given in
\cref{formal}.
...
used. The model is simulated with a fixed-step solver (ode4) and a step size of
...
\subsection{Reference Trajectory and Desired Task} The task is to follow a semicircular
...
(see~\cref{fig:ref_kreis_gesamt}). The semicircle lies in the $XY$ plane. All four joint axes
...
indicates both the spatial deviation from the reference and the velocity mismatch.
  ...
  removes the redundancy of the unit-quaternion parameterization. For $q_{\mathrm{err},v} = 0$,
...
joint of the arm. The actions are real-valued and bounded by the actuator capabilities.
Symmetric torque limits $|\tau_{i}| \leq \tau_{\max}$ for $i = 1,\ldots,4$ are enforced through
a saturation block that clips the commanded torque to the interval
$[-\tau_{\max}, +\tau_{\max}]$. The value of $\tau_{\max}$ is selected according to the robot design
($\tau_{\max} = 2\,\mathrm{N\,m}$ in this work), provided it is large enough to follow the
reference trajectory. The saturation enforces both the actuator limits and an upper bound on the applied
torque for safety reasons. The action space is therefore the continuous box
...
$\Delta t = 0.1\,\mathrm{s}$. With an episode duration of $8.5\,\mathrm{s}$, this gives
...
At each step, all inputs to the reward function are checked for numerical validity. An
...
With the penalty in \eqref{eq:rfail}, all of them did. All reward components were implemented in
...
\textbf{Training Procedure.} An episodic training scheme is used. Each episode covers
...
state variables, integrator states, and logging signals are reset as well.

Within an episode, the agent receives an observation at each step and outputs a torque action.
The simulation runs until the end of the horizon or until one of the terminal conditions of
the reward function occurs. In that case the penalty in \eqref{eq:rfail} is applied and the
...
most algorithms converge or reach a performance plateau. During training, the cumulative
...
\hlone{[R2.3] To make the runs reproducible, MATLAB's Mersenne Twister random number generator
...
\textbf{Evaluation and Metrics.} \hlone{[R2.3] After training, each policy is evaluated
...
\hlone{[R2.3] The KPIs are first computed for each seed over the 30 random episodes. The seed
...
\hlone{[R2.3] Algorithms are compared on the seed level with two-sided Mann--Whitney $U$ tests over
the successful runs. A test is reported only when both configurations have at least four successful
runs. The shares of successful runs are compared with Fisher's exact test.} In both cases,
...
\begin{itemize}
  ...
  \item $\boldsymbol{K_6}$ (\emph{Joint Limit Violation Rate}). Fraction of joint samples
...
In addition to the KPIs above, two training-related metrics are reported. $T_1$ is the
...
\centering
   \includegraphics[width=1\linewidth]{Figures/impuls_monitor.pdf}
    ...
    \texttt{SpaceRobot.slx}. The velocities and orientations of the base and of all
...
appears in the first agent step, after which $p_{\mathrm{tot}}$ stays constant. In the evaluated
episodes, no systematic drift of $p_{\mathrm{tot}}$ was observed, and the deviations remained within the
numerical tolerance of the solver. The deviations were larger for agents with rapidly changing torque
commands, which may be explained by the integration error of the fixed-step solver. The momentum
observer is a diagnostic tool. It is evaluated offline and does not influence the training.

\begin{figure*}[!t]
    \centering    \includegraphics[width=0.75\linewidth]{Figures/spacerobot_slx_colissionmonitor.pdf}
    \caption{Collision monitor subsystem of the Simulink model \texttt{SpaceRobot.slx}. The joint
...
\begin{figure*}[!t]
    ...
    \end{subfigure}
    \caption{Test of the collision monitor with constant joint torques that fold the arm toward
...
\end{figure*}
\textbf{Collision Monitor.} At every agent step, a collision monitor computes the
minimum distance $d_{\min}$ between non-adjacent robot bodies (see
\cref{fig:spacerobot_slx_colissionmonitor}). The monitor is a MATLAB Function
block that applies the \texttt{checkCollision} function of the Robotics System Toolbox \cite{b31}
to the collision geometry of the URDF model. If $d_{\min}$ falls below the safety
...
the penalty in \eqref{eq:rfail}.

The monitor was tested in two cases without a trained agent. In the first case, constant
...
therefore does not always precede the contact. During training, collisions occurred mainly
...
\textbf{Formal Check of the Torque Bound.} In addition to the runtime monitors,
the torque bound of the actuation path was checked formally with Simulink Design Verifier (SLDV) in
property-proving mode \cite{b5}. \cref{fig:verify_tau} shows the verification model
\texttt{verify\_tau.slx}. It contains a saturation block with the same limits $\pm\tau_{\max}$ as
the actuation path of \texttt{SpaceRobot.slx}. The commanded torque vector $\tau$ enters this
...
(\cref{fig:verify_tau_result}). Because the input is unconstrained, the bound holds for any output
...
closed loop. The external disturbance of the stress test in \cref{sec:robustness_eval} is added
after the saturation and is therefore not covered.

\begin{figure}[!b]
	\centering
	\includegraphics[width=\linewidth]{Figures/verify_tau_slx.png}
	\caption{Verification model for Simulink Design Verifier (\texttt{verify\_tau.slx}).
...
\textbf{Scope of the Safety Components.} The mechanisms above cover a defined set of
properties, namely collision events, joint-limit events, bounded torques, and physical consistency
through the momentum check. Only the torque bound is checked formally. The collision
...
penalized the corresponding actions. For TRPO and the optimized PPO agent, no violations of
the monitored constraints were observed in any evaluation episode. The other
...
\textbf{Computational Overhead and Scalability.} The momentum observer and the
...
are reported as metric $T_2$ in \cref{tab:kpi_comp}. They range from about 7 minutes (PG) to
...
execute in the control loop. The default and the optimized PPO agents use a compact policy
...
reduced to the most safety-critical checks (torque saturation and joint-limit monitor) with
...
\begin{figure}[!b]
	...
	\caption{Result of Simulink Design Verifier for the torque-bound property. \hlone{[R2.1] The proof
...
\begin{itemize}
  \item \textit{Policy Gradient (PG).} The REINFORCE algorithm (Monte Carlo policy gradient) with a
  learned state-value function as baseline \cite{b7,b21}. It is included as the simplest
...
processor and 32\,GB of RAM, using MATLAB R2026a with the Reinforcement Learning Toolbox and
...
$\Delta t$. \cref{tab:hyperparameters} lists the resulting values. The reward function, the
...
\begin{table*}[!t]
\centering
\caption{Hyperparameters of the benchmarked agents. The six default agents use the default
values of the Reinforcement Learning Toolbox (R2026a). All agents act every $\Delta t = 0.1\,\mathrm{s}$
and use networks with two hidden layers of 128 ReLU units. In contrast to the default PPO
agent, the policy mean of the optimized PPO agent has no tanh output layer. $\alpha_a$ and
...
\textbf{Evaluation Protocol.} \hlone{[R2.3] The agents were evaluated with the protocol of} \cref{RL}\hlone{. Each of the 60
...
\begin{table}[!t]
\centering
\caption{\hlone{[R2.3] Success of the default agents over ten training runs each. A run is successful when none of its
31 evaluation episodes is terminated by a monitor. The abort rate is the share of terminated episodes among
the 310 evaluation episodes of an agent.} The column ``Aborts by contact'' gives the
...
\textbf{Success Across Seeds.} \hlone{[R2.3] The number of successful runs ranged from two to ten out of ten
...
\begin{table*}[!t]
\centering
\caption{\hlone{[R2.3] Performance of the successful runs of the default agents on the circular
...
\begin{figure*}[!t]
	...
	\caption{Episode rewards during training of the six default agents over ten seeds each.
...
\textbf{Performance of the Successful Runs.} \cref{tab:kpi_comp} \hlone{[R2.3] summarizes the KPIs
...
\hlone{[R2.3] TRPO required 11.3\,min per training run, the default PPO agent 8.4\,min, and PG
...
The learning curves in \cref{fig:training_defaults} show the same pattern. The returns
...
\hlone{[R2.3] The control smoothness $K_7$ and the energy consumption $K_9$ separate the agents in a similar way. TRPO
...
\begin{table*}[!t]
\centering
\caption{\hlone{[R2.3] Seed-level comparison of the successful runs of the default agents with those of TRPO. Each cell
...
The Holm correction covers the three tested agents for each KPI. \hlone{[R2.3] PPO and PG
...
\textbf{Statistical Tests.} \cref{tab:significance} \hlone{[R2.3] compares the successful runs of each default agent
...
\cref{fig:boxplots} \hlone{[R2.3] shows the distribution of four KPIs over the successful runs, and}
...
\begin{figure*}[!t]
  ...
  \end{subfigure}
  \caption{\hlone{[R2.3] Distribution of selected KPIs over the successful runs of each agent. Each point is one run,
...
\begin{figure*}[!t]
  ...
  \begin{subfigure}{0.48\textwidth}
    ...
    \label{fig:ci_k9}
  \end{subfigure}
  \caption{\hlone{[R2.3] Mean and 95\% bootstrap confidence interval of selected KPIs over the successful runs of each
...
\textbf{Summary of the Comparison.} \hlone{[R2.3] With the toolbox default hyperparameters, the comparison can be
summarized as follows:}
\begin{itemize}
  \item \hlone{[R2.3] TRPO was the only default agent that completed all ten runs without a terminated evaluation
  episode. It also reached a low tracking error, although three of its runs were weaker than the others.}
  \item \hlone{[R2.3] The default PPO agent and PG succeeded in only three and two of ten runs. Their successful runs
  tracked the path with an error similar to that of TRPO.}
  \item \hlone{[R2.3] SAC, TD3, and DDPG succeeded in four to six runs. Even in their successful runs, they showed
  significantly larger tracking errors than TRPO and required two to three times longer training.}
\end{itemize}
\hlone{[R2.3] These results suggest that the trust-region constraint of TRPO stabilizes training under the strong
...
dependence.} The PPO agent used so far corresponds to the default configuration of MATLAB's
Reinforcement Learning Toolbox \cite{b4}. The tuning concerns the hyperparameters and
...
\end{figure*}
\textbf{Network Architecture.} The optimized agent keeps the hidden layers of the default
...
In the tuning runs, the selected configuration gave the highest mean episodic reward.
Compared with the default configuration, the optimized agent uses learning rates that are
ten to twenty times smaller, larger rollouts and mini-batches, more epochs per update, and gradient
clipping. Together, these settings limit the size of each policy update. In addition, its
...
\textbf{Training Outcome.} \cref{fig:training_ppo_compare} \hlone{[R2.3] shows the training curves of both
...
\textbf{Success and Performance.} \cref{tab:ppo_optimization_kpi} \hlone{[R2.3] compares the optimized PPO agent with the
...
\textbf{Comparison With TRPO.} \hlone{[R2.3] On the seed level, only the difference in the base
...
\begin{figure*}[t]
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \label{fig:traj_opt}
    \end{subfigure}
    \caption{\hlone{[R2.3] Reference and actual end-effector path in the $XY$ plane for one evaluation episode from the
...
\begin{table*}[t]
	\centering
	\caption{\hlone{[R2.3] Default PPO agent, optimized PPO agent, and TRPO on the circular path. The first row gives the
...
\cref{fig:trajektorie_xy_compare} \hlone{[R2.3] shows the end-effector path of one evaluation episode for each PPO
...
\hlone{[R2.3] Under the tested conditions, the tuning removed the seed dependence of the default PPO agent. The tuned
...
the contribution of each term of the reward function in~\eqref{eq:reward}. \hlone{[R2.4] The
...
$W_{\mathrm{ori}}=200$, $W_{\omega_b}=8$, $W_{v_b}=2$, $W_u=0.02$, $W_d=0.06$). \hlone{[R2.4] In
...
\begin{table*}[t]
    \centering
    \caption{\hlone{[R2.4] Ablation study of the reward components on the circular trajectory. Each row corresponds to the
...
\textbf{Dominant Weights ($W_p$, $W_{\mathrm{ori}}$).} \hlone{[R2.4] Removing the end-effector position weight $W_p$ had
...
\textbf{Remaining Terms.} \hlone{[R2.4] Disabling any of the other five cost weights or one of the two positive reward
...
\textbf{Summary.} \hlone{[R2.4] The ablation indicates that $W_p$ and $W_{\mathrm{ori}}$ are the two dominant components of
...
\hlone{[R2.4] The ablation study in}~\cref{sec:reward_ablation} \hlone{identifies the dominant reward components. A
...
\begin{table*}[t]
    \centering
    \caption{\hlone{[R2.4] One-at-a-time sensitivity analysis of four reward weights of the optimized PPO agent on the
...
\textbf{Optimized PPO Agent.} \hlone{[R2.4] All 40 runs of the eight perturbed configurations succeeded
...
\begin{table}[!t]
    \centering
    \caption{\hlone{[R2.4] Success and tracking error of TRPO, the default PPO agent, and SAC under changed reward weights
...
\textbf{Ranking of the Algorithms.} \cref{tab:reward_ranking} \hlone{[R2.4] shows TRPO, the default PPO
...
\hlone{[R2.4] The order of the default PPO agent and SAC was not stable. SAC had more successful runs
...
\textbf{Summary.} \hlone{[R2.4] Within the tested range from 0.5 to 2 times the nominal
values, the optimized PPO agent succeeded in every run, and the reward weights mainly shifted the balance
between tracking and base stabilization.} \hlone{[R2.4] TRPO succeeded in every run and had the
...
runs.} \hlone{[R2.4] The analysis covers changes of single weights and three algorithms.
...
\hlone{[R2.2, R2.3] The stress tests evaluate the trained policies beyond the nominal simulation assumptions. All 70 policies
...
1.5.} This uniform scaling keeps the mass ratios and the base--manipulator coupling
...
distribution of the model. \hlone{[R2.2, R2.3] The combined case applied the noise, the torque limit
of $0.75\,\tau_{\max}$, the disturbance, and the parameter change at the same time.}

\begin{table*}[t]
    \centering
    \caption{\hlone{[R2.2, R2.3] Stress tests of the trained policies on the circular trajectory, evaluated without retraining.
    ...
    default agents, it gives the number of successful runs out of ten.} ``Parameters
...
\textbf{TRPO and Optimized PPO Agent.} \hlone{[R2.2, R2.3] Both agents completed all runs without a terminated episode under
...
and to 0.02847 for TRPO, about three and two times the nominal values.} One possible reason
is that the scaling weakens every torque command, whereas the torque limits clip only large commands.
\hlone{[R2.2, R2.3] The combined case showed the same increase in $K_2$ together with the increase in
$K_4$ caused by the disturbance. Under every condition, the optimized PPO agent kept a lower mean
tracking error than TRPO.}

\textbf{Other Agents.} \hlone{[R2.2, R2.3] Under the torque limit of $0.3\,\tau_{\max}$, more runs of the other default agents
...
model assumes independent Gaussian noise per channel and step.} The parameter test scales
...
TRPO.} \hlone{[R2.3] On the seed level, the optimized PPO agent had a higher return and lower mean and
...
\hlone{[R2.3] Under the tested conditions, the configurations of TRPO and the optimized PPO
...
monitored safety properties. \hlone{[R2.3] The main observation is that no RL algorithm is universally
...
The results show that an RL-based controller can perform the desired manipulation task in
the simulated space environment under the considered assumptions. \hlone{[R2.3] With the
...
it trained faster.} \hlone{[R2.5] No violations of the monitored safety properties were recorded} for
TRPO and the tuned PPO agent in any evaluation episode.

\hlone{[R2.2, R2.3, R2.4] An additional evaluation on a piecewise-linear end-effector
...
PPO did not prevent the seed dependence observed in} \cref{comparison}\hlone{.} The runtime
monitors (collision monitor and joint-limit monitor) supervise the training. They terminate episodes in
which the policy would violate a monitored constraint and apply a terminal penalty.
...
task-specific and may require re-tuning for different trajectories or mission scenarios. It
was selected in an earlier grid search with a single run per candidate. It also differs from
the default agent in the output layer of the policy mean, which has no tanh bound, and the effects of the
two changes were not separated. The other algorithms were compared only with their toolbox
...
paths.} Both paths lie in one plane, and all joint axes are parallel, so the task is planar
...
\hlone{[R2.3] The comparison is mainly evidence of how robust training is to the choice of seed in this
...
and momentum consistency).} Only the torque bound of the actuation path is checked formally,
...
model, nor an exhaustive formal verification of the mission.} Finally, \emph{no hardware
...
\cref{fig:oos}). The second is the extension to more complex tasks such as three-dimensional
...
authors' testbed for in-space robotized services with virtually simulated microgravity and
...
multibody simulation, runtime safety monitoring, and a formal check of the torque
...
than TRPO, although this difference was not statistically significant.} The runtime monitors
...
for language editing and grammar enhancement. The illustration in \cref{fig:oos} was
...
\bibitem{la2025deep}
T. K. La and E. Guiffo Kaigom, ``Deep learning for model-free prediction of thermal states of
...
E. Guiffo Kaigom and J. Roßmann, ``Value-driven robotic digital twins in cyber--physical
applications,'' \emph{IEEE Trans. Ind. Informat.}, vol.~17, no.~5, pp. 3609--3619, 2021.

\bibitem{kaigom2023metarobotics}
E. Guiffo Kaigom, ``Metarobotics for industry and society: Vision, technologies, and
...
\bibitem{b31} MathWorks, ``checkCollision---Check if robot is in collision,'' Robotics System
...
was born in Frankfurt am Main, Germany, on December 18, 2002. He received the
B.Eng. degree in mechatronics with the overall grade ``very good'' from Frankfurt University of Applied
Sciences, Frankfurt am Main, Germany, in 2026. He is currently pursuing the M.Sc. degree in mechatronics
with a specialization in robotics at Technische Universit\"at Darmstadt, Darmstadt, Germany.

From 2023 to 2026, he was a Student Research Assistant with Frankfurt University of Applied Sciences
in a research project on theoretical quantum mechanics, where he carried out analytical calculations and
validated them with a numerical simulation. He was a Teaching Assistant for Physics~I and~II, Control
Systems, Robotics and Machine Learning, and Human--Robot Interaction. During an internship at
SEW-EURODRIVE, Bruchsal, Germany, he developed a path-planning pipeline for automated guided vehicles
and validated it in simulation and on physical prototypes. In 2026, he completed the ESA Space Standards
Training Course on the ECSS standards system at ESEC-Galaxia. His research interests include
reinforcement learning for robotics, space robotic systems, and simulation-based control methods.

Mr.\ Ermisch has been listed four times on the Dean’s List of Frankfurt University of Applied
Sciences in recognition of his academic performance.
\end{IEEEbiography}
